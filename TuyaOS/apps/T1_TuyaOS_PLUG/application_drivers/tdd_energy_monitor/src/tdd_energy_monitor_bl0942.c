...
STATIC OPERATE_RET __tdd_energy_monitor_bl0942_calibration(BL0942_DRIVER_DATA_T *tdd_data, ENERGY_MONITOR_CAL_PARAMS_T *cal_params)
{
    ...
    TAL_PR_DEBUG("reg_data params: %d,  %d, %d", reg_data.WATT.byte_h, reg_data.WATT.byte_m, reg_data.WATT.byte_l);

    // 新增补码转换逻辑
    if (reg_data.WATT.byte_h & 0x80) { 
        // 负功处理：取补码并转换为有符号数
        temp_val = ((~temp_val) & 0x00FFFFFF) + 1;
        temp_val = -temp_val; // 转换为负数
    }

    if (temp_val != 0 && tdd_data->cal_data.power > 0) 
    {
        cal_params->pf_num = ((FLOAT_T)3600000/16384)*(abs(temp_val) * 100 / tdd_data->cal_data.power)/256;
    } else {
        cal_params->pf_num = 0;
    }
    TAL_PR_DEBUG("temp params: %d,  %d", temp_val, tdd_data->cal_data.power);
