...
UINT32_T tdl_energy_monitor_calibration(ENERGY_MONITOR_HANDLE_T handle, ENERGY_MONITOR_CAL_DATA_T cal_data, OUT ENERGY_MONITOR_CAL_PARAMS_T *cal_params)
{
    ...
    TAL_PR_NOTICE("cal_params: %d, %d, %d, %d", cal_params->voltage_period, cal_params->current_period, cal_params->power_period, cal_params->pf_num);
...
#if 1
    TAL_PR_NOTICE("--->");
    TAL_PR_NOTICE("---> voltage_period: def: %-6u, exp: %-6u, cal : %u", def_params.voltage_period, exp_params.voltage_period, cal_params->voltage_period);
    TAL_PR_NOTICE("---> current_period: def: %-6u, exp: %-6u, cal : %u", def_params.current_period, exp_params.current_period, cal_params->current_period);
    TAL_PR_NOTICE("---> power_period:   def: %-6u, exp: %-6u, cal : %u", def_params.power_period,   exp_params.power_period,   cal_params->power_period);
    TAL_PR_NOTICE("---> pf number:      def: %-6u, exp: %-6u, cal : %u", def_params.pf_num,   exp_params.pf_num,   cal_params->pf_num);
#endif

    TAL_PR_NOTICE("--->");
    err_percent = GET_ERR_PERCENT(cal_params->voltage_period, exp_params.voltage_period);
    TAL_PR_NOTICE("---> voltage_period error percent: %d", err_percent);
    tmp_err_percent = GET_ERR_PERCENT(cal_params->current_period, exp_params.current_period);
    TAL_PR_NOTICE("---> current_period error percent: %d", tmp_err_percent);
    ...
    TAL_PR_NOTICE("---> power_period   error percent: %d", tmp_err_percent);
    if (err_percent < tmp_err_percent) {
        err_percent = tmp_err_percent;
    }
    tmp_err_percent = GET_ERR_PERCENT(cal_params->pf_num, exp_params.pf_num);
    TAL_PR_NOTICE("---> pf number      error percent: %d", tmp_err_percent);
    if (err_percent < tmp_err_percent) {
        err_percent = tmp_err_percent;
    }
    TAL_PR_NOTICE("--->");
