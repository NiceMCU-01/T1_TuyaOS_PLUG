...
***********************variable define**********************
***********************************************************/
STATIC INT_T sg_last_raw_level[TUYA_GPIO_NUM_MAX] = {[0 ... TUYA_GPIO_NUM_MAX - 1] = -1};
...
STATIC OPERATE_RET __tdd_create_gpio_button(IN TDL_BUTTON_OPRT_INFO *dev)
{
    ...
    if(p_gpio_local->mode == BUTTON_TIMER_SCAN_MODE){
        ...
        if(p_gpio_local->level == TUYA_GPIO_LEVEL_HIGH){
        ...
        if(p_gpio_local->level == 0) {
            gpio_irq_cfg.mode = TUYA_GPIO_IRQ_FALL;
        } else {
            gpio_irq_cfg.mode = TUYA_GPIO_IRQ_RISE;
        }
...
STATIC OPERATE_RET __tdd_read_gpio_value(TDL_BUTTON_OPRT_INFO *dev, UCHAR_T *value)
{
    ...
    if(OPRT_OK == ret){
        if (p_local_cfg->pin < TUYA_GPIO_NUM_MAX && sg_last_raw_level[p_local_cfg->pin] != result) {
            sg_last_raw_level[p_local_cfg->pin] = result;
            TAL_PR_NOTICE("button gpio[%d] raw=%d active=%d logical=%d",
                          p_local_cfg->pin, result, p_local_cfg->level, (p_local_cfg->level == result) ? 1 : 0);
        }
        //处理有效电平逻辑
...
OPERATE_RET tdd_gpio_button_register(CHAR_T *name, BUTTON_GPIO_CFG_T *gpio_cfg)
{
    ...
    TAL_PR_NOTICE("tdd_gpio_button_register succ name:%s pin:%d active:%d mode:%d",
                  name, gpio_cfg->pin, gpio_cfg->level, gpio_cfg->mode);
    return ret;
}
